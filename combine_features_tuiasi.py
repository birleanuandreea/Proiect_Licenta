...
def merge(folder: str, output: str) -> None:
    ...
    for fpath, cols in config.items():
        ...
        if n_rows is None:
            ...
            print(f"\n[EROARE] '{fpath.name}' are {len(df_raw)} randuri, "
                  f"dar fisierele anterioare aveau {n_rows}.")
            print("  Toate CSV-urile trebuie sa corespunda aceluiasi set de emailuri.")
            sys.exit(1)
